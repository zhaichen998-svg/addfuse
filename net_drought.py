...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, bias=False):
        ...
        self.fc1 = nn.Conv2d(in_features, hidden_features * 2, kernel_size=1, bias=bias)
        self.fc2 = nn.Conv2d(hidden_features, in_features, kernel_size=1, bias=bias)
...
class DetailFeatureExtraction(nn.Module):
    def __init__(self, dim, num_layers=3):
        super().__init__()
        self.layers = nn.Sequential(*[DetailNode(dim) for _ in range(num_layers)])
...
class Restormer_Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.base_layer = BaseFeatureExtraction(dim, heads[2], ffn_expansion_factor, bias)
